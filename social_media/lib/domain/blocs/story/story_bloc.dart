...
class StoryBloc extends Bloc<StoryEvent, StoryState> {
  ...
  Future<void> _selectedImagePreview(
      OnSelectedImagePreviewEvent event, Emitter<StoryState> emit) async {
    emit(state.copyWith(image: event.image));
  }

  Future<void> _addNewStory(
      OnAddNewStoryEvent event, Emitter<StoryState> emit) async {
    try {
      emit(LoadingStory());
